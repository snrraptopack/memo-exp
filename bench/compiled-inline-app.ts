/**
 * @file compiled-inline-app.ts
 * Benchmark app compiled from inline JSX list rows ({data.map(item => <li ... />)}).
 * Uses AppInline root ID for isolated entity registration.
 */
...
export function createCompiledInlineApp() {
  ...
  const region0 = MD.createListRegion<RowData>(ul0, "AppInline/data", (item, rowId) => {
    let $s0: any, $s1: any, $s2: any, $t: any;
