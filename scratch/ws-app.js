...
function NotificationsPanel(_id2, _parent2) {
  ...
  const _when = _MD.createCondRegion(_ul, _id2 + "/when0", () => _MDD.resolvedValuesError([_MDD.readResolvedValueForRender(_MDD.sourceRef("./session.ts#notifications"))]) ? 0 : _MDD.resolvedValuesPending([_MDD.readResolvedValueForRender(_MDD.sourceRef("./session.ts#notifications"))]) ? 1 : 2, [() => {
    let _value3;
    const _update3 = () => {
      _MD.setProps(_id2 + "/when0" + "/ErrorRow", [{
        error: _MDD.resolvedValuesError([_MDD.readResolvedValueForRender(_MDD.sourceRef("./session.ts#notifications"))]),
        retry: _retryCallback
      }]);
    };
    const _retryCallback = () => _MDD.retryResolvedValues([_MDD.readResolvedValueForRender(_MDD.sourceRef("./session.ts#notifications"))]);
