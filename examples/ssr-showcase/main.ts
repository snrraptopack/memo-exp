 ...
 *      `restoreState` on the extension store so that `hydrate` (called
 *      by `mount` when options.hydration is set) can install the payload
 ...
 *
 *   2. `mount` is called in hydration mode (`options.hydration`). It reads
