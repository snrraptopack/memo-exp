  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <div>
        <a
          route-to="/repos"
          className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-stone-900 hover:bg-stone-800 text-stone-300 hover:text-emerald-400 border border-stone-800 text-xs font-semibold transition-all"
        >
      ...
      {/* Repo Details Header */}
      <div if={repoResource.pending && !repoResource.data} className="p-6 rounded-3xl bg-stone-900/60 border border-stone-800 animate-pulse h-48">
        <div className="h-6 bg-stone-800 rounded w-1/3 mb-4"></div>
        <div className="h-4 bg-stone-800/60 rounded w-2/3 mb-2"></div>
        <div className="h-4 bg-stone-800/40 rounded w-1/2"></div>
      </div>
      ...
      <div else className="p-6 sm:p-8 rounded-3xl bg-stone-900/90 border border-stone-800 shadow-2xl space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <img
              src={repoResource.data?.owner.avatar_url}
              alt={owner}
              className="w-14 h-14 rounded-2xl border border-stone-700 shadow-lg"
            />
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-mono text-stone-400">{owner} /</span>
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold text-stone-100 mt-0.5">
                {name}
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="px-4 py-2 rounded-xl bg-stone-800 hover:bg-stone-700 text-stone-200 text-xs font-bold border border-stone-700 transition-all flex items-center gap-1.5"
            >
              <span>GitHub ↗</span>
            </a>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-stone-800/80">
          <div className="p-3.5 rounded-xl bg-stone-950 border border-stone-800/80">
            <span className="text-[11px] text-stone-500 font-mono block">STARS</span>
            <span className="text-lg font-bold text-amber-400 font-mono">
              ★ {repoResource.data?.stargazers_count.toLocaleString()}
            </span>
          </div>
          <div className="p-3.5 rounded-xl bg-stone-950 border border-stone-800/80">
            <span className="text-[11px] text-stone-500 font-mono block">FORKS</span>
            <span className="text-lg font-bold text-stone-200 font-mono">
              🍴 {repoResource.data?.forks_count.toLocaleString()}
            </span>
          </div>
          <div className="p-3.5 rounded-xl bg-stone-950 border border-stone-800/80">
            <span className="text-[11px] text-stone-500 font-mono block">OPEN ISSUES</span>
            <span className="text-lg font-bold text-emerald-400 font-mono">
              ⚡ {repoResource.data?.open_issues_count.toLocaleString()}
            </span>
          </div>
          <div className="p-3.5 rounded-xl bg-stone-950 border border-stone-800/80">
            <span className="text-[11px] text-stone-500 font-mono block">LAST ACTIVITY</span>
            <span className="text-xs font-semibold text-stone-300 font-mono block mt-1">
      ...
      <div className="p-6 rounded-3xl bg-stone-900/80 border border-stone-800 shadow-xl space-y-4">
        ...
        <div if={issuesResource.pending && !issuesResource.data} className="space-y-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-16 rounded-xl bg-stone-950/60 border border-stone-800 animate-pulse"></div>
          ))}
        </div>
        ...
        <div else className="space-y-3">
          {issuesResource.data?.map((issue) => (
            <div
              key={issue.id}
              className="p-4 rounded-xl bg-stone-950/90 border border-stone-800 hover:border-stone-700 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3"
            >
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-sm font-bold text-stone-200 hover:text-emerald-400 transition-colors"
                  >
                    {issue.title}
                  </a>
                </div>
